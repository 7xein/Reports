...
export interface WarrantyBranchRow {
  branch: string;
  overall: number;           // total sales
  withoutWarranty: number;   // portion of total without a warranty
...
export function SalesWarrantyDashboard({
  ...
  const woShare    = overall > 0 ? (withoutW / overall) * 100 : 0;
  ...
  const cards = [
    { lab: 'Total Sales',            val: overall,  foot: `all branches · ${cap}`,            accent: '#1a1a1a' },
    { lab: 'Sales Without Warranty', val: withoutW, foot: `${woShare.toFixed(0)}% of revenue`, accent: '#888888' },
  ];
  ...
  return (
    <>
      ...
      {/* Sales by branch — total bars with % of target */}
      <div className="bg-white rounded-lg shadow-sm mb-4" style={{ padding: '24px 26px' }}>
        <div className="text-sm font-bold uppercase tracking-wide text-ink-muted mb-5">Sales by Branch</div>
        {ordered.map((r) => {
          ...
          return (
            <div key={r.branch} className="mb-[18px] last:mb-0">
              ...
              <div className="h-4 bg-surface rounded-full overflow-hidden">
                <div className="h-full rounded-full" style={{ width: `${fillPct.toFixed(1)}%`, background: BRANCH_COLORS[r.branch] ?? '#999' }} />
              </div>
            </div>
          );
        })}
